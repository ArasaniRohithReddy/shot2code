...
| File | Field | Meaning |
|---|---|---|
| `desktop/package.json` | `0.3.1` | **The release version.** Bump this. |
...
The git tag (`v0.3.1`) and the GitHub release name mirror
...
2. Move the `## [Unreleased]` entries in [`CHANGELOG.md`](../CHANGELOG.md) into a
   ...
   bottom.
3. Commit (for example `chore(release): 0.3.1`) and push `main`.
...
3. Commit the checksums to `docs/releases/v<version>/SHA256SUMS.txt` (see
   [v0.3.1](releases/v0.3.1/SHA256SUMS.txt) for the format) and link them from
